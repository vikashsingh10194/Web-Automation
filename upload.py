...
DEFAULT_REVIEW_SERVER = "coderviaintl.appspot.com"

# Max size of patch or base file.
MAX_UPLOAD_SIZE = 1900 * 1024
...
group.add_option("--cc", action="store", dest="cc",
                 metavar="CC", default="codereview@via.com",
...
group.add_option("--git_committed_only", action="store_true", default=False,
                 dest='git_committed_only',
                 help="limits code reviews to committed changes only")
# Perforce-specific
...
class VersionControlSystem(object):
  ...
  def GenerateDiff(self, args):
    print "In first def of GenerateDiff"
  ...
  def GetBaseFiles(self, diff):
    ...
    b = raw_input("Do you want to upload all the files ? (y/n)")
    if b == 'y' :

      dictf = {}
      if (os.path.isfile('ignore.txt') == True) :
      ...
      for line in diff.splitlines(True):
        ...
        # instead of '/'.
          ...
          if filename in dictf.keys() :
            continue
          if ('.env' in filename) or ('.properties' in filename) :
            continue
      ...
      dictinc = {}
      inc = raw_input("Do you want to use include.txt (y/n)?")
      if inc == 'y':
        if (os.path.isfile('include.txt') == True) :
          ...
          for line in diff.splitlines(True):
            if line.startswith('Index:') or line.startswith('Property changes on:'):
              unused, filename = line.split(':', 1)
              filename = filename.strip().replace('\\', '/')
              if filename in dictinc.keys() :
                files[filename] = self.GetBaseFile(filename)
          return files
        else :
          print "include.txt doesn't exist! \n Please select individual files."
	  ...
	  if filename in dictf.keys() :
            continue
          if ('.properties' in filename) or ('.env' in filename) :
            continue
#          files[filename] = self.GetBaseFile(filename)
          else :
            a = raw_input("Do you want to upload file "+filename+" (y/n)?")
            if a == 'y':
              files[filename] = self.GetBaseFile(filename)
      return files
  ...
  def UploadBaseFiles(self, issue, rpc_server, patch_list, patchset, options,
    ...
    for key in files :
      required_patches[key]=patches[key]

    
    for filename in required_patches.keys():
...
class GitVCS(VersionControlSystem):
  ...
  def GenerateDiff(self, extra_args):
    ...
    if self.options.git_find_copies:
      similarity_options = ["-l100000", "-C%s%%" % self.options.git_similarity]
  ...
  def GetFileContent(self, file_hash):
    ...
    data, err, retcode = RunShellWithReturnCodeAndStderr(["git", "show", file_hash],
                                            universal_newlines=False)
    if retcode:
      # bad object would indicate that the hash is for an uncommitted change.
      if not self.options.git_committed_only and re.search("bad object %s" % file_hash, err):
        # if allowing uncommitted changes, then there is no content to return, so return None
        # It will be calculated by diffs.
        return None
      else:
  ...
  def GetBaseFile(self, filename):
    ...
    #remove below line and uncomment above lines for "commit first, upload after" use case
    # new_content = self.GetFileContent(hash_after)
...
class MercurialVCS(VersionControlSystem):
  ...
  def GenerateDiff(self, extra_args):
    print "In fifth def of GenerateDiff"
...
def RealMain(argv, data=None):
  ...
  #not entering either if nor else
  if verbosity >= 3:
...
#  print files
  ...
  if options.issue:
    ...
    issueprompt = "CoderVIA Issue number (Press enter for new issue): "
    options.issue = raw_input(issueprompt).strip()
    if options.issue:
      form_fields.append(("issue", options.issue))
    if not options.issue:
      reviewerprompt = "Reviewer email ids (comma seperated) :"
      options.reviewers = raw_input(reviewerprompt).strip()
      if options.reviewers:
        form_fields.append(("reviewers", options.reviewers))
  ...
  for e in ar :
    if e.startswith('Index:') or e.startswith('Property changes on:'):
      unused, filename = e.split(':', 1)
      f = filename.replace('\n','').replace(' ','').replace('\r','')
      if f in files.keys():
        lc = cnta+1

        while (lc<l and((not (ar[lc].startswith('Index:'))) and (not (ar[lc].startswith('Property changes on:'))))):
          lc = lc+1
        for i in range (cnta,lc) :
          str = str + ar[i]
    cnta=cnta+1
  ...
  required_patches = []
  for file in files :
#    a = raw_input('Do you want to upload file '+file+'(y/n)?')
#    if a== 'y':
    required_files[file]=files[file]
    for i in range(0,len(patches)) :
       if patches[i][1] == file :
         required_patches = required_patches + [[patches[i][0],patches[i][1]]]
  if not options.download_base:
    vcs.UploadBaseFiles(issue, rpc_server, required_patches, patchset, options, required_files)
  ...
  rpc_server.Send("/" + issue + "/upload_complete/" + (patchset or ""),payload=payload)
