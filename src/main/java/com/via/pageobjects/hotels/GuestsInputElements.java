...
public class GuestsInputElements extends PageHandler {

  private final String PAGE_NAME = "guestsInput";
  private RepositoryParser repositoryParser;

  public GuestsInputElements(String testId, WebDriver driver, RepositoryParser repositoryParser) {
    super(testId, driver);
  ...
  // Gets the pageObject and modifies the string "toModify1" to the target
  // first string in locator value and "toModify2" to second string
  public PageElement modifyPageElementTwice(String objectName, String modification1,
      String modification2) {
    PageElement element = repositoryParser.getPageObject(PAGE_NAME, objectName);
    PageElement modifiedElement = new PageElement(element);
    String locatorValue = modifiedElement.getLocatorValue();
    String modifiedLocatorValue = StringUtils.replace(locatorValue, "toModify1", modification1);
    modifiedLocatorValue = StringUtils.replace(modifiedLocatorValue, "toModify2", modification2);
    modifiedElement.setLocatorValue(modifiedLocatorValue);
    return modifiedElement;
  }
  ...
  // Guests details validation
  public WebElement confirmContactEmail() {
    return findElement(repositoryParser, PAGE_NAME, "confirmContactEmail");
  }

  public WebElement confirmContactPhone() {
    return findElement(repositoryParser, PAGE_NAME, "confirmContactPhone");
  }

  public WebElement confirmProceedPayBtn() {
    return findElement(repositoryParser, PAGE_NAME, "confirmProceedPayBtn");
  }
}
